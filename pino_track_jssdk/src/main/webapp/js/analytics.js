(function(){
	//cookie 其实就是java里的map， 操作map的工具类
	var CookieUtil = {
	...
	var tracker = {
			// config  配置信息
			clientConfig: {
				serverUrl: "http://mini1/BfImg.gif",
				sessionTimeout: 360, // 360s -> 6min    		会话的过期时间
				maxWaitTime: 3600, // 3600s -> 60min -> 1h		最长的等待时间
				ver: "1"
			},

			cookieExpiresTime: 315360000000, // cookie过期时间，10年   永不过期
            ...
            // 这里面的value值就对应着在session里的name（sid，uuid）  和value（例如e_pv，e_crt 就是定义的事件名称）
			keys: {
				pageView: "e_pv",
            ...
            //category: 自定义事件名称，action：自定义事件动作，map：其他参数，duration：事件持续时间
